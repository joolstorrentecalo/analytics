...
WITH dim_date AS (
  ...
  SELECT 
    ...
    mart_crm_opportunity.report_segment,
    mart_crm_opportunity.parent_crm_account_business_unit,
    mart_crm_opportunity.report_geo,
    mart_crm_opportunity.report_region,
    mart_crm_opportunity.report_area,
    ...
    WHERE sdr_bdr_user_id IS NOT NULL 
      AND mart_crm_opportunity.stage_1_discovery_date >= '2022-02-01' 
  ...
  LEFT JOIN sales_dev_opps 
    ON sfdc_lead.converted_opportunity_id = sales_dev_opps.dim_crm_opportunity_id 
  WHERE sfdc_lead.converted_contact_id IS NOT NULL 
  ...
  LEFT JOIN {{ ref('dim_crm_user') }} 
    ON mart_crm_event.booked_by_employee_number = dim_crm_user.employee_number 
  LEFT JOIN dim_date 
  ...
  WHERE waterfall_person_id IS NULL 
    OR activity_to_sao_days > 90 --adds back in the opps that are being discarded due to a too long delay from activity on the lead for that lead to be credited with SAO creation
  ...
  SELECT 
    ...
    sales_dev_user_hierarchy_opportunities.dim_crm_user_id AS sales_dev_rep_user_id,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_role_name,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_email,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_user_full_name,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_title,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_department,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_team,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_is_active,
    sales_dev_user_hierarchy_opportunities.crm_user_sales_segment,
    sales_dev_user_hierarchy_opportunities.crm_user_geo,
    sales_dev_user_hierarchy_opportunities.crm_user_region,
    sales_dev_user_hierarchy_opportunities.crm_user_area,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_employee_number,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_direct_manager_id,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_manager_full_name,
    sales_dev_user_hierarchy_opportunities.sales_dev_manager_email,
    sales_dev_user_hierarchy_opportunities.sales_dev_manager_employee_number,
    sales_dev_user_hierarchy_opportunities.sales_dev_manager_user_role_name,
    sales_dev_user_hierarchy_opportunities.sales_dev_leader_id,
    sales_dev_user_hierarchy_opportunities.sales_dev_leader_user_role_name,
    sales_dev_user_hierarchy_opportunities.sales_dev_rep_leader_full_name,
    sales_dev_user_hierarchy_opportunities.sales_dev_leader_employee_number,
    sales_dev_user_hierarchy_opportunities.sales_dev_leader_email,
    sales_dev_user_hierarchy_activities.dim_crm_user_id AS activity_sales_dev_rep_user_id,
    sales_dev_user_hierarchy_activities.sales_dev_rep_role_name AS activity_sales_dev_rep_role_name,
    sales_dev_user_hierarchy_activities.sales_dev_rep_email AS activity_sales_dev_rep_email,
    sales_dev_user_hierarchy_activities.sales_dev_rep_user_full_name AS activity_sales_dev_rep_full_name,
    sales_dev_user_hierarchy_activities.sales_dev_rep_manager_full_name AS activity_sales_dev_manager_full_name,
    sales_dev_user_hierarchy_activities.sales_dev_rep_leader_full_name AS activity_sales_dev_leader_full_name
  ...
  LEFT JOIN dim_sales_dev_user_hierarchy sales_dev_user_hierarchy_opportunities
    ON opp_to_lead.sdr_bdr_user_id = sales_dev_user_hierarchy_opportunities.dim_crm_user_id 
      AND opp_to_lead.stage_1_discovery_date = sales_dev_user_hierarchy_opportunities.snapshot_date
  LEFT JOIN dim_sales_dev_user_hierarchy sales_dev_user_hierarchy_activities
    ON activity_final.dim_crm_user_id = sales_dev_user_hierarchy_activities.dim_crm_user_id 
      AND activity_final.activity_date = sales_dev_user_hierarchy_activities.snapshot_date
  ...
  LEFT JOIN dim_sales_dev_user_hierarchy sales_dev_user_hierarchy_opportunities
    ON opps_missing_link.sdr_bdr_user_id = sales_dev_user_hierarchy_opportunities.dim_crm_user_id 
    AND opps_missing_link.stage_1_discovery_date = sales_dev_user_hierarchy_opportunities.snapshot_date
  LEFT JOIN mart_crm_account 
