SELECT
    ...
    CASE
        WHEN
            ...
            AND opportunity.competitors <> 'None'
            THEN true
        ELSE false
    END
    ...
    CASE
        WHEN
            ...
            OR opportunity.competitors = 'None'
            THEN true
        ELSE false
    END
        AS no_comp_opps,
...
FROM
    restricted_safe_common_mart_sales.mart_crm_opportunity
        AS opportunity
LEFT JOIN common.dim_date AS close
    ON opportunity.close_date = close.date_actual
WHERE
    close.current_fiscal_year - opportunity.close_fiscal_year <= 3
