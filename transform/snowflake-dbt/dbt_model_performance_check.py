...
def run():
    ...
    for res in result_data.get("results"):
        print(
            f"{res.get('unique_id')} {result_data.get('metadata').get('generated_at')} {res.get('adapter_response').get('rows_affected')} {res.get('execution_time')}"
        )
    ...
    # query = """
    # select * from pempey_prod.workspace_data.data_type_mock_table
    # """

    # query_result = query_executor(query_engine, query)
    # print(query_result)
