...
* [ ] New/altered metrics impact check:  need to investigate MRs with the label `Data Warehouse::Impact Check` and assign to `@gitlab-data/engineers`. Review the [**list**](https://gitlab.com/groups/gitlab-org/-/merge_requests?scope=all&state=all&label_name[]=Data%20Warehouse%3A%3AImpact%20Check&draft=no&.approved_by_usernames[]=Any&reviewer_username=gitlab-data%2Fengineers) and see the impact that new metrics bring when executed. For the detailed guideline, check handbook page [](https://handbook.gitlab.com/handbook/business-technology/data-team/how-we-work/triage/#automated-service-ping-issue).
